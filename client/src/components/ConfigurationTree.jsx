  ...
  const loadRootConfigurations = async () => {
    ...
    try {
      ...
      let rootConfigs = (response.data.configs || []).filter(config => {
        ...
        // Debug: Store user configs info for display
        if (config.type === "USER") {
          window.debugUserConfigs = window.debugUserConfigs || [];
          window.debugUserConfigs.push({
            name: config.name,
            created_by: config.created_by,
            current_user_id: user?.id,
            matches: config.created_by === user?.id
          });
        }

        return isRootLevel || isUserConfigByCurrentUser;
      });
