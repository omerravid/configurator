...
class ConfigurationService {
  ...
  static async getValueAtPath(configIdOrName, path, minimal = false) {
    ...
    if (minimal) {
      // Return the raw value directly (no provenance wrappers in minimal mode)
      console.log(`Minimal value returned - Type: ${typeof current}, Value:`, current);
      return current;
    }
  ...
  static async updateConfiguration(configId, { data, description }, updaterId) {
    ...
    // Validate schema enforcement for updates - TEMPORARILY DISABLED FOR INSTANCE AND USER
    if (data && config.type !== "PRODUCT" && config.type !== "INSTANCE" && config.type !== "USER" && config.parent_id) {
