...
// Copy entire database using backup+restore approach
router.post("/databases/copy-database", authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    const username = req.user?.username || 'admin';
    console.log(`Starting database copy from "${sourceDatabase}" to "${targetDatabase}" using backup+restore approach`);
    ...
    const br = new BackupRestore();
    const backupName = br.generateBackupName(username, `copy-from-${sourceDatabase}`);
    ...
    res.json({
      success: true,
      message: `Database "${sourceDatabase}" copied to "${targetDatabase}" successfully using backup+restore approach`,
      backupFile: backupResult.file,
    ...
    res.status(500).json({
      success: false,
      error: error.message || "Failed to copy database"
    });
  }
});
